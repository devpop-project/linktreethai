...
export default function Template1({ profile, links, products, handleLinkClick, isDashboardPreview }: TemplateProps) {
  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-5 px-4 py-6 text-[#1E1B4B] font-sans antialiased relative z-10" style={{
      ...
      <div className="bg-white/95 border border-purple-100/90 rounded-[32px] overflow-hidden shadow-sm text-center backdrop-blur-md">
        ...
        <div className="p-6 pt-0 space-y-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-black tracking-tight text-[#1E1B4B] break-words" style={{ color: profile.text_color || undefined }}>
              {profile.full_name || profile.username}
            </h1>
            <p className="text-xs font-bold text-purple-600 mt-0.5 font-mono" style={{ color: profile.text_color || undefined }}>@{profile.username}</p>
          </div>

          {profile.bio && (
            <p className="text-xs sm:text-sm text-slate-600 leading-relaxed max-w-xs mx-auto break-words" style={{ color: profile.text_color || undefined }}>
              {profile.bio}
            </p>
          )}
            ...
            >
              <div className="flex items-center gap-3.5 overflow-hidden pr-2">
                {link.logo_url ? (
                  ...
                  <div style={{ backgroundColor: link.icon_bg_color || undefined }} className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${!link.icon_bg_color ? (!link.bg_color ? 'bg-purple-50 text-purple-600' : 'bg-white/20 text-white') : ''}`}>
