...
export default function Template7({
  ...
  const [activeTab, setActiveTab] = useState<"links" | "shop" | "lead">(
    "links",
  );
  ...
  const handleSubmitLead = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLeadSentMsg(
        "✅ ส่งข้อความติดต่อเรียบร้อยแล้ว! ทางร้านจะติดต่อกลับโดยเร็วที่สุดครับ",
      );
      setLeadForm({
        name: "",
        phone: "",
  ...
  return (
    <div
      className="w-full max-w-md mx-auto space-y-5 px-4 py-6 text-black font-sans antialiased bg-[#FEFCE8] rounded-3xl border-[3.5px] border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] relative z-10"
      style={{
        backgroundColor: profile.bg_color || undefined,
        ...(profile.inner_bg_image_url &&
        profile.inner_bg_image_url !== profile.bg_image_url
          ? {
    ...
    >
      ...
      <div
        className="border-[3px] border-black rounded-2xl p-6 text-center flex flex-col items-center space-y-3 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] relative"
        style={{ backgroundColor: profile.card_bg_color || "#FDE047" }}
      >
        ...
        <div>
          <h1
            className="text-2xl font-black tracking-tight text-black break-words"
            style={{ color: profile.text_color || undefined }}
          >
            {profile.full_name || profile.username}
          </h1>
          <p
            className="text-xs font-black text-purple-700 font-mono mt-0.5"
            style={{ color: profile.text_color || undefined }}
          >
            @{profile.username}
          </p>
        ...
        {profile.bio && (
          <p
            className="text-xs sm:text-sm text-black font-bold leading-relaxed max-w-xs mx-auto bg-white p-3 rounded-xl border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
            style={{ color: profile.text_color || undefined }}
          >
            {profile.bio}
          </p>
      ...
      <div className="flex bg-white border-[3px] border-black p-1 rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] font-mono text-xs">
        <button
          ...
          style={{
            backgroundColor:
              activeTab === "links"
                ? profile.tab_active_color || profile.card_bg_color || "#34D399"
                : undefined,
          }}
          className={`flex-1 py-2 rounded-xl font-black flex items-center justify-center gap-1 transition ${
            activeTab === "links"
              ? "text-black border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
              : "text-black"
          }`}
        >
        ...
        <button
          ...
          style={{
            backgroundColor:
              activeTab === "shop"
                ? profile.tab_active_color || profile.card_bg_color || "#FDE047"
                : undefined,
          }}
          className={`flex-1 py-2 rounded-xl font-black flex items-center justify-center gap-1 transition ${
            activeTab === "shop"
        ...
        <button
          ...
          style={{
            backgroundColor:
              activeTab === "lead"
                ? profile.tab_active_color || profile.card_bg_color || "#C084FC"
                : undefined,
          }}
          className={`flex-1 py-2 rounded-xl font-black flex items-center justify-center gap-1 transition ${
            activeTab === "lead"
          ...
          {links && links.length > 0 ? (
            links.map((link) => (
              <button
                ...
                style={{
                  backgroundColor:
                    link.bg_color || profile?.custom_button_color || undefined,
                  color:
                    link.text_color ||
                    (link.bg_color
                      ? "#FFFFFF"
                      : profile?.custom_button_text_color) ||
                    undefined,
                }}
                ...
                <div className="flex items-center gap-3.5 overflow-hidden pr-2">
                  {link.logo_url ? (
                    <img
                      src={link.logo_url}
                      alt={link.title}
                      className="w-10 h-10 object-cover rounded-xl shrink-0 border-2 border-black"
                    />
                  ) : (
                    <div
                  ...
                  <div className="overflow-hidden">
                    <p
                      className="text-xs sm:text-sm font-black truncate leading-snug"
                      style={{
                        color:
                          link.text_color ||
                          (link.bg_color
                    ...
                    {link.subtitle && (
                      <p className="text-[11px] font-bold opacity-80 truncate mt-0.5">
                        {link.subtitle}
                      </p>
                    )}
                  </div>
            ...
            <div className="text-center text-xs font-bold text-black py-6 bg-white rounded-2xl border-[3px] border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
          ...
          {products && products.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {products.map((prod) => (
                <div
                  key={prod.id}
                  className="bg-white border-[3px] border-black rounded-2xl p-4 flex flex-col justify-between space-y-3 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
                >
                  <div className="space-y-2">
                    <div
                      className="w-full h-32 rounded-xl overflow-hidden relative border-2 border-black"
                      style={{
                        backgroundColor: profile.card_bg_color || "#FDE047",
                      }}
                    >
                      {prod.image_url ? (
                        <img
                    ...
                    <div>
                      ...
                      {prod.description && (
                        <p className="text-[11px] font-semibold text-slate-600 line-clamp-2 mt-0.5">
                          {prod.description}
                        </p>
                      )}
                    </div>
            ...
            <div className="text-center text-xs font-bold text-black py-6 bg-white rounded-2xl border-[3px] border-black">
          ...
          {leadSentMsg ? (
            <div className="p-6 bg-[#34D399] border-2 border-black rounded-2xl font-black text-xs text-black text-center space-y-2 animate-in zoom-in-95 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
              ...
              <p className="text-[11px]">
                ระบบได้ส่งข้อมูลเข้าสู่กล่องข้อความลีดเรียบร้อยแล้ว
              </p>
            </div>
          ) : (
            <form
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setLeadForm({ ...leadForm, line_id: e.target.value })
                    }
      ...
      {!profile.hide_branding && (
        <div className="text-center pt-2 pb-4 font-mono">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white border-2 border-black text-black text-[11px] font-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]"
          >
