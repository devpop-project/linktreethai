...
export default function Template8({
  ...
  return (
    <div
      className="w-full max-w-md mx-auto space-y-5 px-4 py-6 text-white font-sans antialiased relative z-10"
      style={{
    ...
    >
      ...
      <div className="bg-white/[0.08] border border-white/20 rounded-[38px] overflow-hidden shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] text-center flex flex-col items-center backdrop-blur-2xl relative p-6 space-y-4">
        ...
        <div>
          <h1
            className="text-2xl font-black tracking-tight text-white break-words"
        ...
        {profile.bio && (
          <p
            className="text-xs sm:text-sm text-slate-200 leading-relaxed max-w-xs mx-auto break-words"
            style={{ color: profile.text_color || undefined }}
          >
            {profile.bio}
          </p>
      ...
      {activeTab === "links" && (
        <div className="space-y-3">
          {links && links.length > 0 ? (
                ...
                <div className="flex items-center gap-3.5 overflow-hidden pr-2">
                  {link.logo_url ? (
                    <img
                      src={link.logo_url}
                      alt={link.title}
                      className="w-10 h-10 object-cover rounded-2xl shrink-0 border border-white/30 shadow-md"
                    />
                  ) : (
                    <div
                      style={{
                      ...
                      className="w-10 h-10 bg-cyan-500/20 text-cyan-300 rounded-2xl flex items-center justify-center shrink-0 border border-cyan-400/30"
                    >
            ...
            <div className="text-center text-xs text-slate-400 py-6 bg-white/[0.05] rounded-3xl border border-white/15">
          ...
          {products && products.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {products.map((prod) => (
                <div
                  key={prod.id}
                  className="bg-white/[0.08] border border-white/20 rounded-3xl p-4 flex flex-col justify-between space-y-3 shadow-lg hover:border-white/40 transition backdrop-blur-xl"
                >
      ...
      {!profile.hide_branding && (
        <div className="text-center pt-2 pb-4">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 border border-white/20 text-slate-300 hover:text-white text-[11px] font-semibold transition shadow-sm backdrop-blur-md"
          >
