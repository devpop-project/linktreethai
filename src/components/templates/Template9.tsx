...
export default function Template9({
  ...
  return (
    <div
      className="w-full max-w-md mx-auto space-y-5 px-4 py-6 text-white font-sans antialiased bg-[#090D16]/90 border border-indigo-500/40 rounded-[36px] shadow-[0_0_40px_rgba(99,102,241,0.25)] backdrop-blur-2xl relative z-10"
      style={{
    ...
    >
      ...
      <div className="text-center space-y-3 pt-2">
        ...
        <div>
          <h1
            className="text-xl sm:text-2xl font-black tracking-tight text-white flex items-center justify-center gap-1.5 break-words"
          ...
          <p
            className="text-xs font-bold text-indigo-400 font-mono mt-0.5"
            style={{ color: profile.text_color || undefined }}
          >
            @{profile.username}
          </p>
        ...
        {profile.bio && (
          <p
            className="text-xs sm:text-sm text-slate-300 leading-relaxed max-w-xs mx-auto break-words"
            style={{ color: profile.text_color || undefined }}
          >
            {profile.bio}
          </p>
      ...
      <div className="flex bg-slate-950/80 border border-indigo-500/40 p-1 rounded-2xl shadow-xl backdrop-blur-md text-xs font-bold">
        <button
          ...
          className={`flex-1 py-2 rounded-xl flex items-center justify-center gap-1.5 transition ${
            activeTab === "links"
              ? "bg-gradient-to-r from-indigo-500 to-purple-600 text-white shadow font-black"
      ...
      {activeTab === "links" && (
        <div className="space-y-3">
          <h3 className="text-xs font-extrabold text-indigo-300 uppercase tracking-wider px-2 flex items-center gap-1.5">
            <Zap className="w-4 h-4 text-indigo-400" /> DIRECT CHANNELS &amp;
            LINKS
          </h3>
          {links && links.length > 0 ? (
            links.map((link) => (
              <button
                ...
                className={`w-full p-4 rounded-2xl font-bold text-left flex items-center justify-between transition-all group shadow-md hover:scale-[1.01] ${
                  !link.bg_color
                    ? "bg-[#131C2E] border border-indigo-500/30 hover:border-indigo-400"
                ...
                <div className="flex items-center gap-3.5 overflow-hidden pr-2">
                  {link.logo_url ? (
                    <img
                      src={link.logo_url}
                      alt={link.title}
                      className="w-10 h-10 object-cover rounded-xl shrink-0 border border-white/20 shadow-md"
                    />
                  ) : (
                    <div
                      style={{
                        backgroundColor: link.icon_bg_color || undefined,
                      }}
                      className="w-10 h-10 bg-indigo-500/20 text-indigo-300 rounded-xl flex items-center justify-center shrink-0 border border-indigo-500/30"
                    >
                  ...
                  <div className="overflow-hidden">
                    <p
                      className="text-xs sm:text-sm font-extrabold truncate leading-snug group-hover:text-indigo-300 transition"
                      style={{
                        color:
                          link.text_color ||
                          (link.bg_color
            ...
            <div className="text-center text-xs text-slate-500 py-6 bg-slate-900 rounded-2xl border border-slate-800">
          ...
          {products && products.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {products.map((prod) => (
                <div
                  key={prod.id}
                  className="bg-gradient-to-b from-[#172033] to-[#101524] border border-indigo-500/30 rounded-2xl p-4 flex flex-col justify-between space-y-3 shadow-lg hover:border-indigo-400 transition"
                >
        ...
        <div className="bg-[#131C2E]/95 border-2 border-indigo-500/40 rounded-[32px] p-5 sm:p-6 shadow-2xl space-y-4 backdrop-blur-2xl text-white">
          <div className="text-center pb-2 border-b border-indigo-500/20">
            ...
            <p className="text-xs text-slate-300 mt-1 font-medium">
              กรอกชื่อ เบอร์โทร และข้อความ
              เพื่อให้เจ้าของโปรไฟล์ติดต่อกลับหรือรับคำปรึกษา
            </p>
          </div>
      ...
      {!profile.hide_branding && (
        <div className="text-center pt-2 pb-4">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-900 border border-indigo-500/30 text-indigo-200 hover:text-white text-[11px] font-semibold transition shadow-sm"
          >
