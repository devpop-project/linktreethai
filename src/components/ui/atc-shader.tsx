...
export default function ShaderDemo_ATC({
  ...
  useEffect(() => {
    ...
    if (!gl) {
      if (pre) pre.textContent = "WebGL2 not available"
      return
    }
    ...
    const resize = () => {
      ...
      const w = Math.floor((rect.width || canvas.clientWidth || window.innerWidth) * dpr)
      const h = Math.floor((rect.height || canvas.clientHeight || window.innerHeight) * dpr)
