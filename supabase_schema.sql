...
-- ==============================================================================
-- MIGRATION: TIKTOK LIVE TTS FEATURE (200 POINTS / 30 DAYS, FREE FOR MASTER VIP)
-- ==============================================================================
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'tiktok_tts_expires_at') THEN
        ALTER TABLE public.profiles ADD COLUMN tiktok_tts_expires_at TIMESTAMP WITH TIME ZONE;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'tiktok_username') THEN
        ALTER TABLE public.profiles ADD COLUMN tiktok_username TEXT;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'tiktok_tts_settings') THEN
        ALTER TABLE public.profiles ADD COLUMN tiktok_tts_settings JSONB DEFAULT '{"voice_speed": 1.0, "voice_pitch": 1.0, "voice_volume": 1.0, "read_gift": true, "filter_profanity": true, "prefix_format": "{name} พูดว่า {comment}"}'::jsonb;
    END IF;
END $$;

-- RPC for unlocking TikTok Live TTS with 200 points (30 days)
CREATE OR REPLACE FUNCTION public.unlock_tiktok_tts_with_points(user_id UUID)
RETURNS JSONB AS $$
DECLARE
    current_pts INT;
    cur_exp TIMESTAMP WITH TIME ZONE;
    new_exp TIMESTAMP WITH TIME ZONE;
BEGIN
    SELECT points, tiktok_tts_expires_at INTO current_pts, cur_exp FROM public.profiles WHERE id = user_id;
    IF current_pts IS NULL OR current_pts < 200 THEN
        RETURN jsonb_build_object('success', false, 'message', 'แต้มสะสมไม่เพียงพอ (ต้องการ 200 แต้ม)');
    END IF;
    IF cur_exp IS NOT NULL AND cur_exp > NOW() THEN
        new_exp := cur_exp + INTERVAL '30 days';
    ELSE
    ...
    SET points = points - 200,
        tiktok_tts_expires_at = new_exp
    WHERE id = user_id;
    RETURN jsonb_build_object('success', true, 'message', 'ปลดล็อกระบบ TikTok Live เสียงอ่านคอมเมนต์สำเร็จ 30 วัน!', 'expires_at', new_exp);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
